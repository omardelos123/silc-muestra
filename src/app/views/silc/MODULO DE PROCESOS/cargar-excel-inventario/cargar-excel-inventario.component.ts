...
@Component({
  selector: 'app-cargar-excel-inventario',
  templateUrl: './cargar-excel-inventario.component.html',
  styleUrls: ['./cargar-excel-inventario.component.scss']
})
export class CargarExcelInventarioComponent implements OnInit {
  ...
  bool_proceder = false;
  matriz: any[];
  FormatoExcel: any[] = ["Numero Parte","descripcion","cant_disponible","cant_autorizada","min_permitida","precio_unitario_base"];
  ...
  cargarExcel(ev) {
    ...
    reader.onload = (event) => {
      ...
      for (var R = range.s.r; R <= range.e.r; ++R) {
        ...
        var campo2 = XLSX.utils.encode_cell({ c: 1, r: R }); // construct A2 reference for cell
        var campo3 = XLSX.utils.encode_cell({ c: 2, r: R }); // construct A3 reference for cell
        var campo4 = XLSX.utils.encode_cell({ c: 3, r: R }); // construct A4 reference for cell
        var campo5 = XLSX.utils.encode_cell({ c: 4, r: R }); // construct A5 reference for cell
        var campo6 = XLSX.utils.encode_cell({ c: 5, r: R }); // construct A6 reference for cell
  ...
  enviar() {
    ...
    this.flag = true;
    this.rest.PROC_cargar_catalogo_repuesto(this.solicitud_envio_matriz)
      .then((respuesta) => {
