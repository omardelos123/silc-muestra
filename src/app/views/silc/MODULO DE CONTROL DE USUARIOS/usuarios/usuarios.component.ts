...
export class UsuariosComponent implements OnInit {
 ...
 crear() {
   const arreglo = {
     titulo: "CREAR USUARIOS", estatus: "crear"
   }
   const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
   this.router.navigate(['./silc/nuevo-usuarios', parametros]);
 }

 modificar(datos: any) {
   const arreglo = {
     titulo: "MODIFICAR USUARIO", estatus: "modificar", datos: datos
   }
   const 
   parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo))));
   this.router.navigate(['./silc/nuevo-usuarios', parametros]);
 ...
 eliminar_modal(datos: any) {
   ...
   this.nombre_usuario = datos.nombre;
   this.apellido_usuario = datos.apellido;
   this.solicitud.id_usuario = datos.id_usuario;
   this.solicitud.sw_activo = 0;

   swal({
    title: "Desea eliminar el registro? \n " + this.nombre_usuario +" "+this.apellido_usuario,
    type: "warning",
 ...
 setPage(page: number) {
   ...
   this.usuarios = this.allItems.slice(this.pager.startIndex, this.pager.endIndex + 1);
