...
@Component({
  selector: 'app-almacen-compania',
  templateUrl: './almacen-compania.component.html',
  styleUrls: ['./almacen-compania.component.scss']
})
export class AlmacenCompaniaComponent implements OnInit {
  ...
  almacen_compania: any = [];
  nombre_almacen_compania: string = '';
  ...
  constructor( private router: Router, private rest: RestService,
   ...
   crear() {
     const arreglo = {
       titulo: "ALMACEN DE COMPAÑIA", estatus: "crear"
     }
     const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
     this.router.navigate(['./silc/nuevo-almacen-compania', parametros]);
   }
  
   modificar(datos: any) {
     const arreglo = {
       titulo: "ALMACEN DE COMPAÑIA", estatus: "modificar", datos: datos
     }
     const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
     this.router.navigate(['./silc/nuevo-almacen-compania', parametros]);
   }
   ...
   detalle(datos: any) {
     const arreglo = {
       titulo: "ALMACEN DE COMPAÑIA", estatus: "detalle", datos: datos
     }
     const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
     this.router.navigate(['./silc/nuevo-almacen-compania', parametros]);
   }
   ...
   eliminar_modal(datos: any) {
     ...
     this.nombre_almacen_compania = datos.nombre;
     this.solicitud.id_almacen_compania = datos.id_almacen_compania;
     this.solicitud.sw_activo = 0;
     swal({
      title: "Desea eliminar el registro? \n " + this.nombre_almacen_compania,
      type: "warning",
   ...
   setPage(page: number) {
     ...
     this.almacen_compania = this.allItems.slice(this.pager.startIndex, this.pager.endIndex + 1);
