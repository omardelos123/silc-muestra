...
export class ModelosComponent implements OnInit {
  ...
  */
  ...
  crear() {
    const arreglo = {
      titulo: "CREAR MODELO", estatus: "crear"
    }
    const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
    this.router.navigate(['./silc/nuevo-modelos', parametros]);
  }

  modificar(datos: any) {
    const arreglo = {
      titulo: "MODIFICAR MODELO", estatus: "modificar", datos: datos
    }
    const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
    this.router.navigate(['./silc/nuevo-modelos', parametros]);
  }
  ...
  detalle(datos: any) {
    const arreglo = {
      titulo: "DETALLE MODELO", estatus: "detalle", datos: datos
    }
    const parametros = btoa(unescape(encodeURIComponent(JSON.stringify(arreglo)))) // btoa(JSON.stringify(arreglo));
    this.router.navigate(['./silc/nuevo-modelos', parametros]);
  }
  ...
  eliminar_modal(datos: any) {
    ...
    this.nombre_modelo = datos.nombre;
    this.solicitud.id_modelo = datos.id_modelo;
  ...
  setPage(page: number) {
    ...
    this.modelo = this.allItems.slice(this.pager.startIndex, this.pager.endIndex + 1);
